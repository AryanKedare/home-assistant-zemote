...
def _verify_login(email: str, password: str) -> bool:
    """Validate credentials against the Zemote account backend."""
...
def _fetch_account_data(email: str) -> dict:
    """Fetch all devices for the given account from the Zemote cloud."""
    cognito     = boto3.client("cognito-identity", region_name=AWS_REGION_COGNITO)
    identity_id = cognito.get_id(IdentityPoolId=IDENTITY_POOL_ID)["IdentityId"]
    creds       = cognito.get_credentials_for_identity(IdentityId=identity_id)["Credentials"]
    session = boto3.Session(
    ...
    hubs = dynamo.Table(TABLE_MASTER).query(
        KeyConditionExpression=Key("email").eq(email)
    ).get("Items", [])
    serial_to_hub = {h["serialNumber"]: h for h in hubs if "serialNumber" in h}

    modules = dynamo.Table(TABLE_MODULE_DATA).query(
        KeyConditionExpression=Key("email").eq(email)
    ).get("Items", [])
    ...
    for mod in modules:
        ...
        for item in mod.get("lfmData") or []:
            ...
            devices.append({
                "applianceId": sub_id, "moduleId": appliance_id,
                "serialNumber": serial, "name": _prefixed_name(room, item.get("name") or sub_id),
                "channelKey": sub_type, "dimmable": dimmable,
                "platform": platform, "hubName": hub_name, "roomName": room,
            })
        ...
        for item in mod.get("powerModuleData") or []:
            ...
            devices.append({
                "applianceId": sub_id, "moduleId": appliance_id,
                "serialNumber": serial, "name": _prefixed_name(room, item.get("name") or sub_id),
                "channelKey": sub_type, "dimmable": False,
                "platform": "switch", "hubName": hub_name, "roomName": room,
            })
        ...
        for item in mod.get("curtainData") or []:
            ...
            devices.append({
                ...
                "serialNumber": serial, "name": _prefixed_name(room, item.get("name") or sub_id),
                "channelKey": sub_type, "dimmable": False,
                "platform": "cover", "hubName": hub_name, "roomName": room,
            })
        ...
        if sur:
            ...
            if sur_type and sur_type != DIMMER_NA and raw_name != DIMMER_NA and sur_id not in seen_ids:
                ...
                devices.append({
                    "applianceId": sur_id, "moduleId": appliance_id,
                    "serialNumber": serial, "name": _prefixed_name(room, raw_name),
                    "channelKey": sur_type, "dimmable": False,
                    "platform": SUR_TYPE_MAP.get(sur_type, "switch"),
                    "hubName": hub_name, "roomName": room,
                })
        ...
        if rgb:
            ...
            if rgb_id and rgb_id not in seen_ids:
                ...
                devices.append({
                    "applianceId": rgb_id, "moduleId": appliance_id,
                    "serialNumber": serial, "name": _prefixed_name(room, raw_name),
                    "channelKey": rgb.get("type", "RGB"), "dimmable": True,
                    "platform": "light", "hubName": hub_name, "roomName": room, "isRgb": True,
                })
