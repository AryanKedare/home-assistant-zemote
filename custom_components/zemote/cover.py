...
class ZemoteCover(CoverEntity):
    ...
    def is_closed(self) -> bool | None:
        if self._last_cmd == CMD_CLOSE:
            return True
        if self._last_cmd == CMD_OPEN:
            return False
        return None
    ...
    def open_cover(self, **kwargs: Any) -> None:
        self._last_cmd = CMD_OPEN
        self._hub.set_channel(self._serial, self._channel, CMD_OPEN)
        self.schedule_update_ha_state()

    def close_cover(self, **kwargs: Any) -> None:
        self._last_cmd = CMD_CLOSE
        self._hub.set_channel(self._serial, self._channel, CMD_CLOSE)
        self.schedule_update_ha_state()

    def stop_cover(self, **kwargs: Any) -> None:
        self._last_cmd = CMD_STOP
        self._hub.set_channel(self._serial, self._channel, CMD_STOP)
        self.schedule_update_ha_state()
